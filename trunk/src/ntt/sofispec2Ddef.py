def skysofifrom2d(fitsfile,skyfile):
    ...
    shift=ntt.efoscspec2Ddef.checkwavelength_arc(aa1,yy1,skyaa,skyff,'','')    
    delete('_new3.fits')
    return shift
...
def sofispecreduction(files,_interactive,_doflat,listflat,_docross,_verbose=False):
    ...
    files=ntt.sofiphotredudef.sortbyJD(files)  # if they are not sorted the fieldlist dict could crash
    outputlist=[]
    setup=[]
    fieldlist={}
    OBID={}
...
#                     _type='notgood'
        if not _type:
            ...
            if _grism not in fieldlist: fieldlist[_grism]={}
        ...
        for _grism in flats[_date]:
            ...
            for img in flats[_date][_grism]:
                ...
                if readkey3(hdr,'lamp3'):
                    ...
                    if popen(img)[0].data.mean()>=2000:  _type='ON'
                    else:                                _type='OFF'
                ...
                print '\n### image '+str(img)
                answ=raw_input('ON/OFF/REJECT/STOP ['+str(_type)+']  ok (ON[n]/OFF[f]/r/s) ['+_type+'] ? ')
                if not answ: answ=_type
                if answ in ['ON','on','n']: _type='ON'
                if answ in ['OFF','off','f']: _type='OFF'
                if answ in ['s','S','STOP','stop','Stop']: _type='stop'
                if answ in ['r','R','reject']: _type='r'
                if _type in ['ON','OFF']:  flat[_type].append(img)
                elif _type=='stop':
            ...
            if len(flat['ON'])==len(flat['OFF']) and  len(flat['OFF'])>=2:
                ...
                for ii in range(0,len(flat['OFF'])):
                    delete('flat_'+str(_date)+'_'+str(_grism)+'_'+str(MJDtoday)+'_'+str(ii)+'.fits')
                    iraf.imarith(flat['ON'][ii],'-',flat['OFF'][ii],result='flat_'+str(_date)+'_'+str(_grism)+'_'+str(MJDtoday)+'_'+str(ii)+'.fits',verbose='no') 
                    ff.write('flat_'+str(_date)+'_'+str(_grism)+'_'+str(MJDtoday)+'_'+str(ii)+'.fits\n')
                ff.close()
                masterflat='flat_'+str(_date)+'_'+str(_grism)+'_'+str(MJDtoday)+'.fits'
                delete(masterflat)
                _order='80'
                iraf.ccdred.flatcombine(input='@_flatlist',output=masterflat, combine='median', rdnoise=_rdnoise, gain=_gain, ccdtype='')
                hdr=readhdr(masterflat)
                matching = [s for s in hdr.keys() if "IMCMB" in s]
                for imcmb in matching:   aaa=iraf.hedit(masterflat,imcmb,delete='yes',update='yes',verify='no',Stdout=1)
                delete('_flatlist')
                correctcard(masterflat)
                if masterflat not in outputlist:     outputlist.append(masterflat)
                ...
                iraf.specred.response(masterflat,normaliz=masterflat+'[100:900,*]',\
                                  response='n'+masterflat,interac=_interact, thresho='INDEF', sample = '*', naverage=2,\
                                  function='spline3', low_rej=3,high_rej=3,order=_order, niterat=20, grow=0, graphic='stdgraph', mode='q')
                listflat.append('n'+masterflat)
                if 'n'+masterflat not in outputlist:  outputlist.append('n'+masterflat)
                ntt.util.updateheader('n'+masterflat,0,{'FILETYPE':[41203,'normalized flat field'],'TRACE1':[masterflat,'Originating file']})
#                ntt.util.updateheader('n'+masterflat,0,{'TRACE1':[masterflat,'']})
                ...
                for img in flattot:
                    ...
                    ntt.util.updateheader('n'+masterflat,0,{'PROV'+str(num):[readkey3(readhdr(img),'ARCFILE'),'Originating file']})
        ...
        if _grism in lamps:
            ...
            if arcfile:
                    ...
                    ntt.util.updateheader(arcfile,0,{'FILETYPE':[41104,'pre-reduced 2D arc'],\
                                                     ...
                                                     'TRACE1':[readkey3(readhdr(arcfile),'ARCFILE'),'Originating file']})
...
#                        raw_input('ddd')
                    ...
                    iraf.longslit.reidentify(referenc=arcfile, images= arcfile, interac='NO', section= 'column 10',\
                                     coordli='direc$standard/ident/Lines_XeAr_SOFI.dat', overrid='yes', step=10, newaps= 'yes', nsum=5, nlost=2, mode='h', verbose='no')
                    iraf.longslit.fitcoords(images=re.sub('.fits','',arcfile), fitname=re.sub('.fits','',arcfile), interac='no', combine='yes', databas='database',\
                    ...
                    if identific:
                        ...
                        hedvec={'LAMRMS':[_rms*.1,'residual RMS [nm]'],'LAMNLIN':[_num,'Nb of arc lines used in the fit of the wavel. solution'],\
                                'SPEC_ERR':[(_rms*.1)/sqrt(float(_num)),'statistical uncertainty'],'SPEC_SYE':[0.1,'systematic error']}
                        ntt.util.updateheader(arcfile,0,hedvec)
            else:        sys.exit('Warning: arcfile not found')
#########################################################################################################
        for field in fieldlist[_grism]:
                ...
                listatemp=listaobj[:]
                ###############             flat            ##########################
                if listflat and _doflat:      flat0=ntt.util.searchflat(listaobj[0],listflat)[0]
                else:                         flat0=''
...
#                    num2=listatemp.index(listasub[j])
                    imgout=field+'_'+str(_date)+'_'+str(_grism)+'_'+str(MJDtoday)+'_'+str(listatemp.index(img))+'.fits'
                    print '\n### input image: '+str(img)
                    delete(imgout)
                    ...
                    if _docross:
                        ...
                        correctcard(imgout)
                        ntt.util.updateheader(imgout,0,{'CROSSTAL':['True','']})
                    else:
                        os.system('cp '+img+' '+imgout)
                        correctcard(imgout)
                    if _flatcor:
                        print '### correct for flat field   .....   done'
                        try:     iraf.noao.imred.ccdred.ccdproc(imgout,output='', overscan='no', trim='no', zerocor='no', flatcor=_flatcor, flat=flat0)
                        except:
                            iraf.imutil.imreplace(images=flat0,value=0.01,lower='INDEF',upper=0.01,radius=0)
                            iraf.noao.imred.ccdred.ccdproc(imgout,output='', overscan='no', trim='no', zerocor='no', flatcor=_flatcor, flat=flat0)
                        iraf.noao.imred.ccdred.ccdproc(imgout,output='', overscan='no', trim='yes', zerocor='no', flatcor='no', flat='',trimsec='[30:1000,1:1024]')
                        ntt.util.updateheader(imgout,0,{'FLATCOR':[flat0,'flat correction']})

                    if imgout not in outputlist:  outputlist.append(imgout)
                ...
                if len(listatemp)>=2 and len(listasub)>=2:
                  for j in range(0,len(listatemp)):
                    print '### ',listatemp[j],listasub[j],str(readkey3(readhdr(listatemp[j]),'xcum')),str(readkey3(readhdr(listasub[j]),'xcum')),\
                        str(readkey3(readhdr(listatemp[j]),'JD')),str(readkey3(readhdr(listatemp[j]),'JD'))
                    if _interactive:
                        answ=raw_input('\n### ok [[y]/n] ? ')
                        if not answ: answ='y'
                    else: answ='y'
                    ...
                    if answ=='y':
                        ...
                        image2=raw_input('which image do you want to subtract')
                        num2=listatemp.index(image2)
                    imgoutsub=field+'_'+str(_date)+'_'+str(_grism)+'_'+str(MJDtoday)+'_'+str(num1)+'_'+str(num2)+'.fits'
                    delete(imgoutsub)
                    iraf.images.imutil.imarith(operand1=image1,op='-',operand2=image2, result=imgoutsub,verbose='no')
                    ntt.util.updateheader(imgoutsub,0,{'skysub':[image2,'sky image subtracted'],'FILETYPE':[42115,'pre-reduced frame sky subtracted'],\
                                                       'TRACE1':[image1,'Originating file'],\
                                                       'PROV2':[readkey3(readhdr(image2),'ARCFILE'),'Originating file'],
                ...
                for img in reduced:
                    if arcfile:
                        ...
                        delete('t'+img)
                        iraf.specred.transform(input=img, output='t'+img, minput='', fitnames=re.sub('.fits','',arcfile), databas='database',\
                                           x1='INDEF', x2='INDEF', y1='INDEF', y2='INDEF', flux='yes', mode='h', logfile='logfile')
                        ntt.util.updateheader('t'+img,0,{'ARC':[arcfile,''],'FILETYPE':[42106,'wavelength calibrate 2D frames'],'TRACE1':[img,'Originating file']})
                        ntt.util.updateheader('t'+img,0,{'TRACE1':[img,'Originating file']})
                        ntt.util.updateheader('t'+img,0,{'LAMRMS':[ntt.util.readkey3(hdra,'LAMRMS'),'residual RMS [nm]'],\
...
###########################
                        delete('t'+arcfile)
                        iraf.specred.transform(input=arcfile, output='t'+arcfile, minput='', fitnames=re.sub('.fits','',arcfile), databas='database',\
                                           x1='INDEF', x2='INDEF', y1='INDEF', y2='INDEF', flux='yes', mode='h', logfile='logfile')
                        specred=ntt.util.spectraresolution2(arcfile,50)
                        if specred:  ntt.util.updateheader('t'+img,0,{'SPEC_RES':[specred,'Spectral resolving power']}) 
                        delete('t'+arcfile)
###########################
                        iraf.hedit('t'+img,'TRACE2',delete='yes',update='yes',verify='no',Stdout=1)

                        if 't'+img not in outputlist:  outputlist.append('t'+img)
                        print '\n### 2D frame t'+str(img)+' wavelengh calibrated  ............ done'

                        _skyfile=ntt.__path__[0]+'/standard/ident/sky_'+_grism+'.fits'              ###### check in wavelengh   #########
                        ...
                        if glob.glob(_skyfile) and readkey3(hdr,'exptime')>20.:
                            ...
                            if os.path.isfile(_archive):    imgstart=_archive
                            elif os.path.isfile(_original): imgstart=_original
                            else: imgstart=''
                            if imgstart:
                                delete('_tmp.fits')
                                print imgstart, arcfile
                                iraf.specred.transform(input=imgstart, output='_tmp.fits', minput='', fitnames=re.sub('.fits','',arcfile), databas='database',\
                                                   x1='INDEF', x2='INDEF', y1='INDEF', y2='INDEF', flux='yes', mode='h', logfile='logfile')
                                shift=ntt.sofispec2Ddef.skysofifrom2d('_tmp.fits',_skyfile)
                                zro=popen('_tmp.fits')[0].header.get('CRVAL2')
                                delete('_tmp.fits')
                                if _interactive:
                                    answ=raw_input('do you want to correct the wavelengh calibration with this shift: '+str(shift)+' [[y]/n] ? ')
                                    if not answ: answ='y'
                                else: answ='y'
                                if answ.lower() in ['y','yes']:
                                    ntt.util.updateheader('t'+img,0,{'CRVAL2':[zro+int(shift),''],'shift':[float(shift),'']})
#                                    ntt.util.updateheader('t'+img,0,{'shift':[float(shift),'']})
                        ...
                        try:
                            ...
                            wavelmin=float(readkey3(hdrt,'CRVAL2'))+(0.5-float(readkey3(hdrt,'CRPIX2')))*float(readkey3(hdrt,'CDELT2'))
                            wavelmax=float(readkey3(hdrt,'CRVAL2'))+((float(readkey3(hdrt,'NAXIS2'))+0.5-float(readkey3(hdrt,'CRPIX2')))*float(readkey3(hdrt,'CDELT2')))
                            hedvec={}
                            hedvec['WAVELMIN']=[wavelmin*.1,'[nm] minimum wavelength']
                            hedvec['WAVELMAX']=[wavelmax*.1,' [nm] maximum wavelength']
...
#                            hedvec['APERTURE']=[float(re.sub('slit','',readkey3(hdrt,'slit'))),'aperture width']
    ...
    f=open('logfile_spec2d_'+str(reduceddata)+'_'+str(datenow)+'.raw.list', 'w')
