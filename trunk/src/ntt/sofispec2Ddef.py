...
#    if not lamps:
#        sys.exit('\n### error: spectroscopic lamp not available, add lamps in the directory and try again')
        ...
        for _date in flats:
            for _grism in flats[_date]:
                ...
                if len(flat['ON']) == len(flat['OFF']) and len(flat['OFF']) >= 2:
                    ...
                    for imcmb in matching:   aaa = iraf.hedit(masterflat, imcmb, delete='yes', update='yes',
                                                              verify='no', Stdout=1)
                    delete('_flatlist')
                    print masterflat
    ...
    for _grism in fieldlist:
        ...
        if _grism in lamps:
            ...
            if arcfile:
                ...
                ntt.util.delete('arc_' + datea + '_' + _grism + '_' + str(MJDtoday) + '.fits')

                print arcfile, flat0, _flatcor,_doflat

                if _doflat:
                    iraf.noao.imred.ccdred.ccdproc(arcfile,
                                               output='arc_' + datea + '_' + _grism + '_' + str(MJDtoday) + '.fits',
                                               overscan='no', trim='no', zerocor='no', flatcor=_flatcor, flat=flat0)
                else:
                    os.system('cp '+arcfile+' '+'arc_' + datea + '_' + _grism + '_' + str(MJDtoday) + '.fits')
        ...
        for field in fieldlist[_grism]:
            ...
            for img in reduced:
                if arcfile:
                    ...
                    if specred:
                        ntt.util.updateheader('t' + img, 0, {'SPEC_RES': [specred, 'Spectral resolving power']})
                    delete('t' + arcfile)
                    ###########################
                    ...
                    try:
                        ...
                        hedvec['SPEC_BIN'] = [
                            ((wavelmax * .1) - (wavelmin * .1)) / (float(readkey3(hdr, 'NAXIS2')) - 1),
