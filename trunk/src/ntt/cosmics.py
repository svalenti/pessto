...
def lacos(_input0,output='clean.fits',outmask='mask.fits',gain=1.3,readn=9,xorder=9,yorder=9,sigclip=4.5,sigfrac=0.5,objlim=1,verbose=True,interactive=False):
    ...
    ntt.cosmics.tofits(outmask, np.float32(arrayinput-arrayinput), headerinput,verbose=False)
    ...
    arrayoldoutput, headeroldoutput = ntt.cosmics.fromfits(oldoutput,verbose=False)
...
#    m5 = ndimage.filters.median_filter(_inputarray, size=5, mode='mirror')
    ...
    ntt.cosmics.tofits(firstsel, np.float32(arrayf), headersigmap,verbose=False)
    ...
    ntt.cosmics.tofits(outmask, np.float32(arrayoutmask), headerfirst,verbose=False)
    delete(inputmask)
    arrayinputmask=(1-(10000*arrayoutmask))*arrayoldoutput
    ntt.cosmics.tofits(inputmask, np.float32(arrayinputmask), headerfirst,verbose=False)
    delete(med5)
...
def clean_image(img,cleanimg):
    ...
    try:
	    c = ntt.cosmics.cosmicsimage(array, gain=readkey3(header,'gain'), readnoise=readkey3(header,'ron'), sigclip = 5.0, sigfrac = 0.3, objlim = 5.0,verbose=False)
...
def lacos_im(_input,_output='clean.fits',outmask='mask.fits',gain=1.3,readn=9,xorder=9,yorder=9,sigclip=4.5,sigfrac=0.5,objlim=1,skyval=0,niter=2,verbose=True,interactive=False):
	...
	ntt.cosmics.tofits(oldoutput, np.float32(arrayoldoutput), headerinput,verbose=False)
	# start iterations
	while stop=='no':
		# take second-order derivative (Laplacian) of input image
		# kernel is convolved with subsampled image, in order to remove negative
