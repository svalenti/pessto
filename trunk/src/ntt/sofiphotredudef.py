...
def pesstocombine(imglist,_combine,_rejection,outputimage):
     import ntt
     from ntt.util import delete,readhdr,readkey3,display_image, defsex, updateheader
     from pyraf import iraf
     from numpy import compress, array, round, median, std, isnan, sqrt, argmin, sort, argsort
     from pyfits import open as popen
     import string,os,sys,re
     max_sep=20
     _ron=readkey3(readhdr(imglist[0]),'ron')
     _gain=readkey3(readhdr(imglist[0]),'gain')
     ...
     if len(a)>=50:
          am=array(iraf.proto.fields('detections.cat',fields='4',Stdout=1),float)
          a=a[argsort(am)][0:50]
     ...
     x,y=array(x),array(y)
     if readkey3(readhdr(imglist[0]),'instrume')=='sofi':
          ref_cumoffsetx0 = readkey3(readhdr(imglist[0]),'xcum')
          ref_cumoffsety0 = readkey3(readhdr(imglist[0]),'ycum')
     else:
          ref_cumoffsetx0 = 0
          ref_cumoffsety0 = 0
     goodlist,_offset=[],[]
     for img in imglist:
          if img:
               ...
               if len(b)>=50:
                    bm=array(iraf.proto.fields('detections.cat',fields='4',Stdout=1),float)
                    b=b[argsort(bm)][0:50]
               ...
               if len(xdist) <= 5:    print '\n### Warning: less than 5 stars, check  ? !!! \n '
               print '### measure offsets between frames ',str(xoff),str(yoff),str(round(cumoffsetx+float(xoff),3)),str(round(cumoffsety+float(yoff),3))
               if readkey3(hdr,'instrume')=='sofi':
                         ntt.util.updateheader(img,0,{'HIERARCH ESO SEQ CUMOFFSETX':[round(cumoffsetx+float(xoff),3),'']})
                         ntt.util.updateheader(img,0,{'HIERARCH ESO SEQ CUMOFFSETY':[round(cumoffsety+float(yoff),3),'']})
               goodlist.append(img)
               xoffnew=(float(cumoffsetx)-float(ref_cumoffsetx0))+float(xoff)
               yoffnew=(float(cumoffsety)-float(ref_cumoffsety0))+float(yoff)
               _offset.append(str((-1)*(xoffnew))+'   '+str((-1)*(yoffnew))+'\n') 
     f=open('_offset','w')
     for i in _offset:
          f.write(i)
     f.close()
     ...
     delete('imcombinelog')
     delete(outputimage)
     iraf.images.immatch.imcombine('@_goodlist',outputimage,combine=_combine, reject=_rejection, offset='_offset', masktyp='', rdnoise=_ron,\
                        gain=_gain, zero='mode', logfile='imcombinelog')
     ntt.util.updateheader(outputimage,0,{'M_EPOCH':[False,'TRUE if resulting from multiple epochs']})
     ntt.util.updateheader(outputimage,0,{'SINGLEXP':[False,'TRUE if resulting from single exposure']})
     ntt.util.updateheader(outputimage,0,{'TEXPTIME':[readkey3(hdr,'TEXPTIME')*len(goodlist),'Total integration time of all exposures (s)']})
     ntt.util.updateheader(outputimage,0,{'EXPTIME':[readkey3(hdr,'EXPTIME')*len(goodlist),'Total integration time of all exposures (s)']})
     ntt.util.updateheader(outputimage,0,{'GAIN':[2./3. * len(goodlist) * readkey3(hdr,'gain'),'Total integration time of all exposures (s)']})

     num=0
     for im in goodlist:
          num=num+1
          ntt.util.updateheader(outputimage,0,{'PROV'+str(num):[readkey3(readhdr(im),'ARCFILE'),'Originating file']}) 
          # replace with im if we submit single images
          ntt.util.updateheader(outputimage,0,{'TRACE'+str(num):[im,'Originating file']})
     hdr=readhdr(outputimage)
     matching = [s for s in hdr.keys() if "IMCMB" in s]
     for imcmb in matching:   aaa=iraf.hedit(outputimage,imcmb,delete='yes',update='yes',verify='no',Stdout=1)
     if 'SKYSUB' in hdr.keys(): aaa=iraf.hedit(outputimage,'SKYSUB',delete='yes',update='yes',verify='no',Stdout=1)

     delete('_offset,_goodlist,detection*.txt,detections.cat,detection_sex*')
     return outputimage
...
def registersofi(imglist):
     import ntt
     from ntt.util import delete,readhdr,readkey3, defsex, updateheader
     from pyraf import iraf
     from numpy import compress, array, round, median, std, isnan, sqrt, argmin, sort, argsort
     import pyfits
     import string,os,sys,re
     ...
     if readkey3(hdr0,'instrume')=='sofi':
     ...
     for img in imglist:
          if img:
               ...
               if len(xdist) >= 5:
                    print '### measure offsets between frames ',str(xoff),str(yoff),str(round(cumoffsetx+float(xoff),3)),str(round(cumoffsety+float(yoff),3))
                    ntt.util.updateheader(img,0,{'CRPIX1':[readkey3(hdr,'CRPIX1')+round(cumoffsetx+float(xoff),3),'']})
                    ntt.util.updateheader(img,0,{'CRPIX2':[readkey3(hdr,'CRPIX2')+round(cumoffsety+float(yoff),3),'']})
               else:
                    ntt.util.updateheader(img,0,{'CRPIX1':[readkey3(hdr,'CRPIX1')+round(cumoffsetx,3),'']})
                    ntt.util.updateheader(img,0,{'CRPIX2':[readkey3(hdr,'CRPIX2')+round(cumoffsety,3),'']})

               ntt.util.updateheader(img,0,{'CRVAL1':[readkey3(hdr0,'CRVAL1'),'']})
               ntt.util.updateheader(img,0,{'CRVAL2':[readkey3(hdr0,'CRVAL2'),'']})
...
def pesstocombine2(imglist,_combine,outputimage):
     ...
     listheader=['NAXIS1','NAXIS2','CRVAL1','CRVAL2','CRPIX1','CRPIX2','CD2_2','CD1_1']
     hedvec={}
...
#     hedvec['NCOMBINE']:[len(imglist),'Number of raw science data']
     ...
     header0={'DATE-OBS':[_dateobs,'Date the observation was started (UTC)'],'MJD-OBS':[min(mjdstart),'Start of observations (days)'],\
              'MJD-END':[max(mjdend),'End of observations (days)'],'TELAPSE':[_telapse,'Total elapsed time [days]'],'TMID':[_tmid,' [days] MJD mid exposure'],\
              'ASSON1':[re.sub('.fits','.weight.fits',outputimage),'Name of associated file'],'ASSOC1': ['ANCILLARY.WEIGHTMAP','Associated weigh map image']}
     ntt.util.updateheader(outputimage,0,header0)
#     ntt.util.updateheader(outputimage,0,{'MJD-OBS':[min(mjdstart),'Start of observations (days)']})
#     ntt.util.updateheader(outputimage,0,{'MJD-END':[max(mjdend),'End of observations (days)']})
#     ntt.util.updateheader(outputimage,0,{'ASSON1':[re.sub('.fits','.weight.fits',outputimage),'Name of associated file']})
#     ntt.util.updateheader(outputimage,0,{'ASSOC1':['ANCILLARY.WEIGHTMAP','Associated weigh map image']})
     ########
     if 'SKYSUB' in hdr0.keys(): aaa=iraf.hedit(outputimage,'SKYSUB',delete='yes',update='yes',verify='no',Stdout=1)

     data, hdr = pyfits.getdata(outputimage, 0, header=True)
     dataw, hdrw = pyfits.getdata(re.sub('.fits','.weight.fits',outputimage), 0, header=True)
     delete(re.sub('.fits','.weight.fits',outputimage))
...
def crosstalk(inname,outname):
     ...
     iraf.ctio.imcreate("temp_corr_B.fits",naxis=2,naxis1=1024,naxis2=1024,header='new')     #  calculate the second part of the correction
     ...
     iraf.imutil.imarith("temp_corr_A.fits","+","temp_corr_B.fits","temp_corr.fits", verbose='no')      #  combine the 2 corrections
...
def searchill(flat,listill):
    ...
    if not listill:
        directory=ntt.__path__[0]+'/archive/'+str(_instrume)+'/illumination/'+filter0 
        listill=glob.glob(directory+'/*fits')
    else:   directory=''
    illfile=''
    if listill:
        for ill in listill:
             if string.split(flat,'/')[-1] in readkey3(readhdr(ill),'MKILLUM'):    illfile=ill
...
def skysub(lista,_ron,_gain,_interactive,regi='crreject'):
     ...
     iraf.images(_doprint=0)
     toforget = ['imutil.imarith','ccdred.ccdproc', 'nproto.objmasks','ccdred.flatcombine','imutil.hedit','immatch.imcombine']
     for t in toforget: iraf.unlearn(t)
...
#          delete('fastmask_'+im)
          ...
          delete(re.sub('.fits','_sky.fits',im))
     f.close()
     gg.close()
#     ll.close()
     dd.close()
     ee.close()
     delete('fastsky_'+lista[0])
     delete('sky_'+lista[0])
     listaout=[]
#################################
...
#     for im in lista:  aaa=iraf.hedit(im,'OBJMASK',delete='yes',update='yes',verify='no',Stdout=1)
     iraf.ccdred.flatcombine('@tmplist0',output='fastsky_'+lista[0],rdnoise=_ron, gain=_gain, ccdtype='', combine='average', reject='avsigclip')
#####################
     ...
     ccc=iraf.nproto.objmasks(images='@tmplist_fastsub', objmasks='@tmplist_mask', omtype= 'boolean',\
                        blksize=-16, convolv='block 3 3',hsigma=4, lsigma=3, minpix= 10, ngrow= 2, agrow= 4., logfile='',Stdout=1)
     delete('imcombinelog')
     ...
     iraf.image.immatch.imcombine('@tmplist0',output='sky_'+lista[0],masktyp='!OBJMASK',maskval=0,combine='median', reject=regi,\
                         scale='mode',statsec='[100:800,100:800]', offsets='', rdnoise=_ron,gain=_gain, nlow=1, nhigh=1, logfile='imcombinelog')
     for im in lista:  aaa=iraf.hedit(im,'OBJMASK',delete='yes',update='yes',verify='no',Stdout=1)

     hdr=readhdr('sky_'+lista[0])
     matching = [s for s in hdr.keys() if "IMCMB" in s]
     for imcmb in matching:   aaa=iraf.hedit('sky_'+lista[0],imcmb,delete='yes',update='yes',verify='no',Stdout=1)

     skyfile=['sky_'+lista[0]]
     ntt.util.updateheader('sky_'+lista[0],0,{'FILETYPE':[31117,'sky image']})

     iraf.imutil.imarith('@tmplist0', '-', 'sky_'+lista[0], result='@tmplist_sky', verbose='no')
     num=0
     for im in lista:
          num=num+1
          hedvec={'skysub':['sky_'+lista[0],'sky image subtracted'],'FILETYPE':[32215,'pre-reduced image sky subtracted'],\
                       'TRACE1':[im,''],'MBKG':[mean(popen('sky_'+lista[0])[0].data),'background level']}
          ntt.util.updateheader(re.sub('.fits','_sky.fits',im),0,hedvec)
          ntt.util.updateheader('sky_'+lista[0],0,{'PROV'+str(num):[readkey3(readhdr(im),'ARCFILE'),'Originating file']}) 
          ntt.util.updateheader('sky_'+lista[0],0,{'TRACE'+str(num):[im,'Originating file']})

     delete('tmplist0,tmplist_s,tmplist_mask,tmplist_sky') 
     for img in lista:
#          delete('fastsky_'+img)
#          delete('fastsub_'+img)
...
def skysuboff(listaon,listaoff,_ron,_gain,_interactive,namesky,regi='crreject'):
     ...
     iraf.flatcombine('@tmplist_off',output='fastskyoff.fits',rdnoise=_ron, gain=_gain,ccdtype='', combine='average', reject='avsigclip')
     ...
     for im in listaoff:  ntt.util.updateheader(im,0,{'OBJMASK':[readkey3(readhdr('fastsub_'+im),'OBJMASK'),'mask']})
     iraf.images.immatch.imcombine('@tmplist_off',output=namesky,masktyp='!OBJMASK',maskval=0,combine='median', reject=regi,\
                                        nlow=1,nhigh=2,\
...
def sofireduction(imglist,listill, listflat,_docross,_doflat,_doill,_interactive,_regi='crreject',_verbose=False,method='iraf'):
    ...
    if _verbose: iraf.ccdred.verbose='yes'
    else: iraf.ccdred.verbose='no'
    ...
    for img in imglist:
        ...
        _type=''
        if _filter.lower() in ['grf','gbf']:          _type='spectrum'
        if not _type:
             if _object.lower() =='flat':                _type='flat'
             elif _object.lower() =='dark':              _type='dark'
             elif _object.lower() =='sky':               _type='sky'
        if not _type:
             if str(readkey3(hdr,'xcum'))!='None' and str(readkey3(hdr,'ycum'))!='None':
                  if _filter not in objects:
                       objects[_filter]=[img]
                  else:    objects[_filter].append(img)
    fieldlist={}
    OBID={}
    outputobject=[]
    for _setup in objects:
       for obj in objects[_setup]:
            ...
            if _setup not in fieldlist[nameobj0]:            fieldlist[nameobj0][_setup]=[]
    ...
    if _doflat:
         if listflat:
              for imgflat in listflat:
                   ...
                   if _filter not in flat:        flat[_filter]=[imgflat]
         for _set in objects:
              if _set not in flat:   flat[_set]=ntt.util.searchflat(objects[_set][0],'')[0]
    ill={}
    if _doill:
         for _set in flat:
              if listill:
                   for imgill in listill:
                        if string.split(flat[_set],'/')[-1] in readkey3(readhdr(imgill),'MKILLUM'): #popen(imgill)[0].header.get('MKILLUM'):
                             ill[_set]=imgill
              else:          ill[_set]=ntt.sofiphotredudef.searchill(flat[_set],'')[0]
    fieldlist2={}
    for _set in objects:
              ...
              if _set in fieldlist[field]:
                   if _interactive:
                        ...
                        for image in fieldlist[field][_set]:
                                  try:      print '### ',image,str(readkey3(readhdr(image),'xcum')),str(readkey3(readhdr(image),'ycum'))
                                  except:   print '### ',image,' dither  not define'
                        answ=raw_input('\n### do you want to reduce this object '+str(field)+' and filter '+str(_set)+' [[y],n] ? ')
                        if not answ: answ='y'
                   else: answ='y'
                   if answ in ['YES','yes','y','Y']:
                             ...
                             if _doill and ill[_set]:
                                  ...
                                  else:             _illum=ill[_set]
                             else:            _illum=''
                             if _doflat and flat[_set]:
                                  if str(flat[_set])[0]=='/':
                                       _masterflat=string.split(flat[_set],'/')[-1]
                                       os.system('rm -rf  '+_masterflat)
                                       iraf.images.imutil.imcopy(flat[_set],output=_masterflat, verbose='no')
                                  else:           _masterflat=flat[_set]
                             else:            _masterflat=''
                             for image in fieldlist[field][_set]:
                                  ...
                                  if field not in fieldlist2:                  fieldlist2[field]={}
                                  if _set not in fieldlist2[field]:            fieldlist2[field][_set]=[]
                                  nameobj2=nameobj+'_'+str(_date)      
                                  nameobj2=nameobj2+'_'+str(_set)+'_'+str(MJDtoday)
                                  ...
                                  else:        iraf.images.imutil.imcopy(image,'C'+nameobjnew, verbose='no')
                                  ...
                                  delete(nameobjnew)
                                  iraf.noao.imred.ccdred.ccdproc('C'+nameobjnew,output=nameobjnew, overscan="no", trim="yes", zerocor="no", flatcor=_flatcor,\
                                                    illumco=_illumco, trimsec='[1:1024,1:1007]', biassec='', flat=_masterflat, illum=_illum , Stdout=1)
                                  ...
                                  if _doill and _illum: updateheader(nameobjnew,0,{'ILLUMCOR':[_illum,'illumination correction']})
                                  if _doflat and _masterflat: updateheader(nameobjnew,0,{'FLATCOR':[_masterflat,'flat correction']})
                                  delete('C'+nameobjnew)
         ...
         for fil in fieldlist2[i]:
              ...
              if max(max(array(_xcum)),max(array(_ycum)))<=400 and max(max(array(_xcum)),max(array(_ycum)))>=1:
                   ...
                   if _interactive:
                        ...
                        num=len(lista)
                        if num in [8,12,16,20,24,28,32,36,40]: num=4
                   if len(lista)>num and readkey3(readhdr(lista[0]),'nexp') % num == 0 and len(lista) % num == 0:
                                     # popen(lista[0])[0].header.get('nexp') % num == 0 and len(lista) % num == 0:
                         ...
                         for img in lista:
                             ...
                             ii=ii+1
                             if ii==num:
                                  listaout0,skyfile0=ntt.sofiphotredudef.skysub(listatmp,_ron,_gain,_interactive,_regi)
                                  listaout=listaout+listaout0
                                  skyfile=skyfile+skyfile0
                                  ii=0
                                  listatmp=[]
                   else:         listaout,skyfile=ntt.sofiphotredudef.skysub(lista,_ron,_gain,_interactive,_regi)
                   ...
                   for img in lista:
                        ...
                        if _interactive:
                             ...
                             if abs(int(_ycum[-1]))>400 or abs(int(_xcum[-1]))>400:    answ0=2
                             else:                                                     answ0=1
                             answ=raw_input('is this ON[1] or OFF[2] ? ['+str(answ0)+'] ')
                             if not answ: answ=answ0
                             answ=int(answ)
                             if answ==1:        listaon.append(img)
                             elif answ==2:      listaoff.append(img)
                        else:
                             if abs(int(_ycum[-1]))>400 or abs(int(_xcum[-1]))>400:       listaoff.append(img)
                             else:                                                        listaon.append(img)
                   print '### image ON \n',listaon,'\n image OFF \n',listaoff
                   if _interactive:
                        try:
                             from pylab import plot, ion, show, clf
...
#                             show()
                        ...
                        while asxx=='yes':
                             ...
                             if not num: num=int(len(listaon))
                             else:                                         
                                  if num.isdigit():  num=int(num)
                                  else:                                  
                                       asxx='yes'
                                       print '\n### Warning: value not valid, try again.....\n'
                   else:   num=len(listaon)
...
#                                                                popen(lista[0])[0].header.get('nexp') % num == 0 \
                        ...
                        for i in range(0,len(listaon)/num):
                             ...
                             nn=nn+num
                             mm=mm+num
                             listaout0,skyfile0=ntt.sofiphotredudef.skysuboff(listaon1,listaoff1,_ron,_gain,_interactive,namesky,_regi)
                             listaout=listaout+listaout0
                             skyfile=skyfile+skyfile0
                   elif len(listaon)==num and len(listaoff)==num:
                        listaout,skyfile=ntt.sofiphotredudef.skysuboff(listaon,listaoff,_ron,_gain,_interactive,namesky,_regi)
                   else:  
              ...
              if listaout:
                   ...
                   nameobj=_object_name+'_'+_date+'_'+_filter+'_merge'+'_'+str(MJDtoday)
...
#                   _rejection=_regi
                   ...
                   try:
                        ...
                        hedvec0={'NCOMBINE':[len(listaout)*ncombine,'Number of raw science data'],\
                                     'NOFFSETS':[int(num),'Number of offset positions'],'NUSTEP':[0,'Number of microstep positions'],\
                                     'NJITTER':[int(float(readkey3(readhdr(nameobjnew),'nexp'))/num),'Number of microstep positions'],\
                        ...
                        ntt.util.updateheader(nameobjnew,0,{'FILETYPE':[32216,'combined dithered sofi images']})
                   ...
                   try:
                        ...
                        rmsx3,rmsy3,num3,fwhmgess,ellgess,ccc,rasys3,decsys3,mbkg3=ntt.efoscastrodef.efoscastroloop([nameobjnew],'2mass',False,40,40,20,'rxyscale',100,30,sexvec,True,10,method)
                        if rmsx3 > 2 or rmsy3 > 2:
                             rmsx3,rmsy3,num3,fwhmgess,ellgess,ccc,rasys3,decsys3,mbkg3=ntt.efoscastrodef.efoscastroloop([nameobjnew],'2mass',False,int(20),int(20),\
                   ...
                   if fwhmgess and fwhmgess < 99:
                        hedvec={'PSF_FWHM':[fwhmgess,'Spatial resolution (arcsec)'],\
                                ...
                                'CSYER1':[rasys3,'Systematic error in  (RA_m - Ra_ref)'],'CSYER2':[decsys3,'Systematic error in (DEC_m - DEC_ref)']}
                        ntt.util.updateheader(nameobjnew,0,hedvec)
                        result=ntt.efoscastrodef.zeropoint(nameobjnew,'2mass',False,False)
                        if result:
                             if os.path.isfile(re.sub('.fits','.ph',nameobjnew)): 
                                  if re.sub('.fits','.ph',nameobjnew) not in outputobject: outputobject.append(re.sub('.fits','.ph',nameobjnew)) 
                             print '\n### zeropoint ..... done'
                             for ll in result:
                                  valore='%3.3s %6.6s %6.6s' %  (str(ll),str(result[ll][1]),str(result[ll][0]))
                                  print '### ',valore
                                  ntt.util.updateheader(nameobjnew,0,{'zp'+ll:[str(valore),'']})
                             valore=''
                             ntt.util.updateheader(nameobjnew,0,{'FLUXCAL':['ABSOLUTE','Certifies the validity of PHOTZP']}) 
                        else:   
                             ntt.util.updateheader(nameobjnew,0,{'FLUXCAL':['UNCALIBRATED','Certifies the validity of PHOTZP']})
                             ntt.util.updateheader(nameobjnew,0,{'PHOTSYS':['NULL','Photometric system VEGA or AB']})
                             ntt.util.updateheader(nameobjnew,0,{'PHOTZP':[9999.,'MAG=-2.5*log(data)+PHOTZP']})
                   else:
                        hedvec={'PSF_FWHM':[9999.,'Spatial resolution (arcsec)'],'ELLIPTIC':[9999.,'Average ellipticity of point sources'],\
                                'CUNIT1':['deg','unit of the coord. trans.'],'CUNIT2':['deg','unit of the coord. trans.'],\
                                'CRDER1':[9999.,'Random error (degree)'],\
                                'CRDER2':[9999.,'Random error (degree)'],\
                                'CSYER1':[9999.,'Systematic error in  (RA_m - Ra_ref)'],'CSYER2':[9999.,'Systematic error in (DEC_m - DEC_ref)'],\
                                'FLUXCAL':['UNCALIBRATED','Certifies the validity of PHOTZP'],\
                                'PHOTSYS':['NULL','Photometric system VEGA or AB'], 'PHOTZP':[9999.,'MAG=-2.5*log(data)+PHOTZP']}
                        ntt.util.updateheader(nameobjnew,0,hedvec)
                   if mbkg3:
                        if  readkey3(readhdr(nameobjnew),'FLUXCAL')=='ABSOLUTE':
                             try:
                                  ntt.util.updateheader(nameobjnew,0,{'ABMAGSAT':[float(mbkg3)+float(readkey3(readhdr(nameobjnew),'PHOTZP')),'Saturation limit for point sources (AB mags)']})
                             except:
                                  ntt.util.updateheader(nameobjnew,0,{'ABMAGSAT':[float(readkey3(readhdr(nameobjnew),'PHOTZP')),'Saturation limit for point sources (AB mags)']})
                        else:                                         ntt.util.updateheader(nameobjnew,0,{'ABMAGSAT':[float(mbkg3),'Saturation limit for point sources (AB mags)']})
                   else:                                              ntt.util.updateheader(nameobjnew,0,{'ABMAGSAT':[9999.,'Saturation limit for point sources (AB mags)']})
                   maglim=ntt.util.limmag(nameobjnew)
                   if maglim: ntt.util.updateheader(nameobjnew,0,{'ABMAGLIM':[maglim,'5-sigma limiting AB magnitude for point sources']})
                   else:      ntt.util.updateheader(nameobjnew,0,{'ABMAGLIM':[9999.,'5-sigma limiting AB magnitude for point sources']})
    ...
    for img in outputobject:
       if str(img)[-5:]=='.fits':
            hdr=readhdr(img)
            ######  cancel pc matrix
            if 'PC1_1' in hdr.keys():   aaa=iraf.hedit(img,'PC1_1',delete='yes',update='yes',verify='no',Stdout=1)
            if 'PC2_2' in hdr.keys():   aaa=iraf.hedit(img,'PC2_2',delete='yes',update='yes',verify='no',Stdout=1)
            if 'PC1_2' in hdr.keys():   aaa=iraf.hedit(img,'PC1_2',delete='yes',update='yes',verify='no',Stdout=1)
            if 'PC2_1' in hdr.keys():   aaa=iraf.hedit(img,'PC2_1',delete='yes',update='yes',verify='no',Stdout=1)
            #################
            #  added for DR2
            #print img
            ...
            ntt.util.updateheader(img,0,{'EFFRON':[12.*(1/sqrt(readkey3(hdr,'ndit')*_ncombine))* sqrt( pi/2 ),\
                                                   'Effective readout noise per output (e-)']})

            try:     ntt.util.phase3header(img)  #  phase 3 definitions
            ...
            if readkey3(hdr,'tech'):   ntt.util.updateheader(img,0,{'PRODCATG':['SCIENCE.'+readkey3(hdr,'tech').upper(),'Data product category']})
            if not readkey3(hdr,'pixscale'):   ntt.util.updateheader(img,0,{'pixscale':[0.288,'pixel/scale (arcsec)']})
    f.close()
...
def getreferencepixels(img,imgmerge):
     ...
     iraf.wcsctran('tmp.pix','tmp.coo',img,inwcs='physical',outwcs='world',columns='1 2',formats='%10.6f %10.6f',verbose='yes')
     iraf.wcsctran('tmp.coo','tmp2.pix',imgmerge,inwcs='world',units='degrees degrees',outwcs='logical',columns='1 2',formats='%10.1f %10.1f',verbose='no')
