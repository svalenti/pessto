def pesstocombine(imglist, _combine, _rejection, outputimage):
    ...
    if ntt.util.readkey3(ntt.util.readhdr(imglist[0]), 'instrume') == 'sofi':
        ref_cumoffsetx0 = ntt.util.readkey3(ntt.util.readhdr(imglist[0]), 'xcum')
        ref_cumoffsety0 = ntt.util.readkey3(ntt.util.readhdr(imglist[0]), 'ycum')
    else:
        ref_cumoffsetx0 = 0
        ref_cumoffsety0 = 0
    goodlist, _offset = [], []
    ...
    for imcmb in matching:
        aaa = iraf.hedit(outputimage, imcmb, delete='yes', update='yes', verify='no', Stdout=1)
    if 'SKYSUB' in hdr.keys():
        aaa = iraf.hedit(outputimage, 'SKYSUB', delete='yes', update='yes', verify='no', Stdout=1)

    ntt.util.delete('_offset,_goodlist,detection*.txt,detections.cat,detection_sex*')
    return outputimage
...
def skysub(lista, _ron, _gain, _interactive, regi='crreject'):
    ...
    for im in lista:
        aaa = iraf.hedit(im, 'OBJMASK', delete='yes', update='yes', verify='no', Stdout=1)

    hdr = readhdr('sky_' + lista[0])
    matching = [s for s in hdr.keys() if "IMCMB" in s]
    for imcmb in matching:
        aaa = iraf.hedit('sky_' + lista[0], imcmb, delete='yes', update='yes', verify='no', Stdout=1)
        ...
        for fil in fieldlist2[i]:
            ...
            if max(max(array(_xcum)), max(array(_ycum))) <= 400 and max(max(array(_xcum)), max(array(_ycum))) >= 1:
                ...
                if len(lista) > num and readkey3(readhdr(lista[0]), 'nexp') % num == 0 and len(lista) % num == 0:
                    ...
                    listaout, skyfile = ntt.sofiphotredudef.skysub(lista, _ron, _gain, _interactive, _regi)
    ...
    for img in outputobject:
        if str(img)[-5:] == '.fits':
            ...
            if 'PC1_1' in hdr.keys():
                aaa = iraf.hedit(img, 'PC1_1', delete='yes', update='yes', verify='no', Stdout=1)
            if 'PC2_2' in hdr.keys():
                aaa = iraf.hedit(img, 'PC2_2', delete='yes', update='yes', verify='no', Stdout=1)
            if 'PC1_2' in hdr.keys():
                aaa = iraf.hedit(img, 'PC1_2', delete='yes', update='yes', verify='no', Stdout=1)
            if 'PC2_1' in hdr.keys():
                aaa = iraf.hedit(img, 'PC2_1', delete='yes', update='yes', verify='no', Stdout=1)
            #################
            #  added for DR2
            #print img
            ...
            if readkey3(hdr, 'tech'):   ntt.util.updateheader(img, 0, {
