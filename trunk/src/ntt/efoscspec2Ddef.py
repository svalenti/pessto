...
def skyfrom2d(fitsfile, skyfile, interac=True):
    ...
    shift = ntt.efoscspec2Ddef.checkwavelength_arc(aa1, yy1, skyaa, skyff, 5500, 6500, interac)
    return shift
...
def checkwavelength_arc(xx1, yy1, xx2, yy2, xmin, xmax, inter=True):
    ...
    for shift in range(-500, 500, 1):
        ...
        xxcut = np.compress((np.array(xxnew) >= minimo) & (np.array(xxnew) <= massimo), np.array(xxnew))
        yycut = np.compress((np.array(xxnew) >= minimo) & (np.array(xxnew) <= massimo), np.array(yy2timesyy))
    ...
    for img in files:
        ...
        if _naxis1 != 1030 or _naxis2 != 1030:
            ...
            if not _type:
                ...
                _type = 'objects'
            if not _type:       print '### ', _object, _filter, _grism, _slit, _imagetype
        ...
        if _listbias:
            ...
            if biaslist:
                for _date in biaslist:
                    biaslist[_date] = ntt.efoscphotredudef.rejectbias(biaslist[_date], _interactive)
            ...
            if _doflat:
                ...
                if _listflat:
                    ...
                    for ff in listflat:
                        ...
                        if OBID not in IDflat: IDflat[OBID] = []
                        IDflat[OBID].append(ff)

                    for ID in IDflat.keys():
                        if len(IDflat[ID]) >= 3:
                            _date = ntt.util.readkey3(ntt.util.readhdr(IDflat[ID][0]), 'date-night')
                ...
                if arcfile:
                    if arcfile[0] == '/':
                        ...
                        iraf.ccdproc(arcfile, output='arc_' + nameout0, overscan="no", trim="yes", zerocor=_zerocor,
                                     ...
                                     trimsec=_trimsec0, biassec='', flat=nmasterflat, readaxi='column', Stdout=1)
                        if not os.path.isfile('arc_' + nameout0): os.system('cp ' + arcfile + ' arc_' + nameout0)
                        if 'arc_' + nameout0 not in outputlist:  outputlist.append('arc_' + nameout0)
                ...
                if arcfile:
                    ...
                    if arcref:
                        ...
                        if answ in ['n', 'N', 'no', 'NO', 'No']:
                            yy1 = pyfits.open(arcref)[0].data[:, 400:410].mean(1)
                            xx1 = np.arange(len(yy1))
                            yy2 = pyfits.open(arcfile)[0].data[:, 400:410].mean(1)
                    ...
                    if specred:  ntt.util.updateheader('t' + nameout0, 0,
                                                       {'SPEC_RES': [specred, 'Spectral resolving power']})
                    ntt.util.delete('t' + arcfile)
                    #####################
                    if 't' + nameout0 not in outputlist:  outputlist.append('t' + nameout0)
                    ntt.util.updateheader('t' + nameout0, 0,
                                          {'FILETYPE': [22106, 'wavelength calibrated 2D spectroscopic frame']})
                    ntt.util.updateheader('t' + nameout0, 0, {'TRACE1': [nameout0, 'Originating file']})
                    ntt.util.updateheader('t' + nameout0, 0, {'ARC': [arcfile, '']})
                    ...
                    if glob.glob(_skyfile) and \
                        ...
                        shift = ntt.efoscspec2Ddef.skyfrom2d('t' + nameout0, _skyfile, _interactive)
                        ...
                        if answ.lower() in ['y', 'yes']:
                            ntt.util.updateheader('t' + nameout0, 0, {'CRVAL2': [zro + int(shift), '']})
                            ntt.util.updateheader('t' + nameout0, 0, {'shift': [float(shift), '']})
                    else:
