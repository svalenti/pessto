def efoscreduction(imglist,_interactive,_doflat,_dobias,listflat,listbias,_dobadpixel,badpixelmask,fringingmask,_archive,typefile,filenameobjects,_system,_cosmic,_verbose=False,method='iraf'):
      import ntt
      from ntt.efoscphotredudef import searchbias
      from ntt.util import delete, readhdr,readkey3, display_image, updateheader, searchflat, rangedata,correctcard
      from numpy import argmin, min, abs, sqrt
      import string,os,re,math,sys,shutil,glob,pickle
      ...
      for img in imglist:
        ...
        if not _type:
          ...
          if not _type:
                ...
                if _filter.lower() in ['g782','r784','z623','u640','b639','v641','r642','i705'] and _imagetype == 'IMAGE':
                      if 'sky,flat' in _object.lower():                                _type='flat'
                      elif 'dome' in _object.lower() or 'flat' in _object.lower():     _type='flat dome'
                      if _type=='flat':
...
#              elif 'acquisition' in _catg.lower() or 'focus' in _object.lower() :        _type='not good'
              elif  'focus' in _object.lower() :        _type='not good'
          if not _type:
      ...
      for _filt in filterlist:
            if _filt not in flatlist1.keys():
                  if _filt in flatlist2.keys(): 
                        if len(flatlist2[_filt])>=3: flatlist[_filt]=flatlist2[_filt]
            elif len(flatlist1[_filt])<3:
                  if _filt in flatlist2.keys(): 
                        if len(flatlist2[_filt])>=3: flatlist[_filt]=flatlist2[_filt]
            elif _filt in flatlist1.keys():
                  if len(flatlist1[_filt])>=3: flatlist[_filt]=flatlist1[_filt]
  ...
  ###### masterbias  #################
      if _dobias:
         if not _archive:
            if listbias:          masterbiaslist=listbias
            else:
                  masterbiaslist=[]    
                  if biaslist:
                        for _date in biaslist:
                              ...
                              if len(biaslist[_date])>=3:
                                    ...
                                    try:
                                          ...
                                          iraf.ccdproc('@biaslist',output='@obiaslist', overscan="no", trim="yes", zerocor='no', fixpix='no', ccdtype='', \
                                                             flatcor='no', darkcor='no', biassec='', trimsec=str(_trimsec), readaxi='column', Stdout=1)
                                          iraf.zerocombine('@obiaslist',output=masterbiasfile,combine='median',reject='ccdclip',ccdtype='', process='no',\
                                                                 rdnoise=_rdnoise,gain=_gain,Stdout=1)
                                          correctcard(masterbiasfile)
                                          num=0
                                          for img in  biaslist[_date]:
                                                num=num+1
                                                ntt.util.updateheader(masterbiasfile,0,{'PROV'+str(num):[readkey3(readhdr(img),'ARCFILE'),'Originating file']})
                                                ntt.util.updateheader(masterbiasfile,0,{'TRACE'+str(num):[readkey3(readhdr(img),'ARCFILE'),'Originating file']})
                                                delete('o'+img)
                                          ntt.util.updateheader(masterbiasfile,0,{'M_EPOCH':[False,'TRUE if resulting from multiple epochs']})
                                          ntt.util.updateheader(masterbiasfile,0,{'SINGLEXP':[False,'TRUE if resulting from single exposure']})
                                          ntt.util.updateheader(masterbiasfile,0,{'FILETYPE':[11201,'bias']})

                                          masterbiaslist.append(masterbiasfile)
                                          if masterbiasfile not in outputfile: outputfile.append(masterbiasfile)
                                    except:
                                          ntt.util.writeinthelog('Warning '+str(biaslist[_date])+' problem with this list of bias \n','./logNTT.txt')
                                    ...
                                    if len(flatlist[_filter])>=3:
                                          ...
                                          while answ0!='y':
                                                ...
                                                try:
                                                      ...
                                                      iraf.ccdproc('@'+listaflat,output='@o'+listaflat, overscan='no', trim='yes',  darkcor='no', fixpix='no',\
                                                                         zerocor=_zerocor, flatcor='no',trimsec=str(_trimsec), biassec='',\
                                                                         zero=_bias, readaxi='column', ccdtype='', Stdout=1)
                                                      delete(masterflat)
                                                      iraf.flatcombine('@o'+listaflat,output=masterflat,combine='average',reject='avsigclip',ccdtype='', process='no',\
                                                                             rdnoise=_rdnoise,gain=_gain, statsec='[100:800,100:800]', lsigma=3, hsigma=2, Stdout=1)
                                                      masterflatlist.append(masterflat)
                                                      correctcard(masterflat)
                                                      num=0
                                                      for img in flatlist[_filter]:
                                                            num=num+1
                                                            ntt.util.updateheader(masterflat,0,{'PROV'+str(num):[readkey3(readhdr(img),'ARCFILE'),'Originating file']}) 
                                                            ntt.util.updateheader(masterflat,0,{'TRACE'+str(num):[readkey3(readhdr(img),'ARCFILE'),'Originating file']}) 
                                                            delete('o'+img)
                                                      ntt.util.updateheader(masterflat,0,{'ZEROCOR':[_bias,'']})
                                                      ntt.util.updateheader(masterflat,0,{'M_EPOCH':[False,'TRUE if resulting from multiple epochs']})
                                                      ntt.util.updateheader(masterflat,0,{'SINGLEXP':[False,'TRUE if resulting from single exposure']}) 
                                                      ntt.util.updateheader(masterflat,0,{'FILETYPE':[11202,'flat field']})

                                                      if masterflat not in outputfile: outputfile.append(masterflat)
                                                except:
                                                      ntt.util.writeinthelog('Warning '+str(flatlist[_filter])+' problem with this list of flat \n',\
                                                                                 './logNTT.txt')
                                                ...
                                                if masterflat and _interactive:      
                                                      ...
                                                      if answ.lower() in ['n','no']:
                                                            ...
                                                            else:  sys.exit('error: problem with flat .... exit')
                                                      else: answ0='y'
                                                else: answ0='y'
         else:               masterflatlist=[]
##########################################################################
      if len(masterbiaslist)==0:     masterbiaslist=''
      if len(masterflatlist)==0:     masterflatlist=''
      ######################################
      if _verbose:
            print ''
            print '#############################'
            ...
            print '#############################'
            print ''
      if masterflatlist:   listaout=listaout+masterflatlist        
      if masterbiaslist:   listaout=listaout+masterbiaslist        
      if typefile=='calib': objectlist={}
      for _filter in objectlist:
        for img in objectlist[_filter]:
           ...
           _exptime=readkey3(hdr,'exptime')
           _date=readkey3(hdr,'date-night')
           nameout=ntt.util.name_duplicate(img,str(_object)+'_'+str(_date)+'_'+str(_filter)+'_'+str(MJDtoday),'')
           _bias=''
           if _dobias:
             if masterbiaslist:       _bias=searchbias(img,masterbiaslist)[0]
             if not _bias:            _bias=searchbias(img,'')[0]
           _flat=''
           if _doflat:
             if masterflatlist:       _flat=searchflat(img,masterflatlist)[0]
             if not _flat:            _flat=searchflat(img,'')[0]
             if _bias:   ### bias  ###
                if _bias[0]=='/':
                     os.system('cp '+_bias+' .')
                     _bias=string.split(_bias,'/')[-1]
           ...
           delete(nameout)
           try:
                 iraf.ccdproc(img,output=nameout, overscan="no", trim="yes", zerocor=_zerocor, flatcor='no', darkcor='no',\
                              trimsec=str(_trimsec),  zero=_bias, biassec='', readaxi='column', Stdout=1)
                 try:
                       iraf.ccdproc(nameout,output='', overscan="no", trim="no", zerocor='no', flatcor=_flatcor, darkcor='no',\
                                          flat=_flat, readaxi='column', ccdtype='', Stdout=1)
                 except:
                 ...
                 ntt.util.updateheader(nameout,0,{'FILTER':[readkey3(readhdr(nameout),'filter'),'Filter name'],\
                                                  ...
                                                  'TRACE1':[readkey3(readhdr(nameout),'ARCFILE'),'Originating file']})
                 ...
                 ntt.util.writeinthelog('\n','./logNTT.txt')
                 if nameout not in outputfile: outputfile.append(nameout)
           except:
           ...
           if _dobadpixel:
                 if not badpixelmask:
                       ...
                       delete(badpixelmask)
                       os.system('cp '+ntt.__path__[0]+'/archive/'+str(_instrume)+'/badpixels/badpixel.fits '+badpixelmask)
           ...
           if _cosmic:
                 try:
                       print '\n### cosmic  ..... '
                       ntt.cosmics.lacos_im(nameout,_output='',gain=_gain,readn=_rdnoise,xorder=9,yorder=9,sigclip=4.5,sigfrac=0.5,objlim=1,skyval=0,niter=0,verbose=True,interactive=False)
                       ntt.util.updateheader(nameout,0,{'LACOSMIC':[True,'TRUE if Laplacian cosmic ray rejection has been applied to the image']})
                       print '\n### cosmic  .....  removed '
                 except Exception, e:    print e
           else:
                 ntt.util.updateheader(nameout,0,{'LACOSMIC':[False,'TRUE if Laplacian cosmic ray rejection has been applied to the image']})
           try:  
##########################
                 sexvec=ntt.efoscastrodef.sextractor(nameout)
                 for cat in ['2mass','usnoa2','usnob1']:
                       rmsx3,rmsy3,num3,fwhmgess,ellgess,ccc,rasys3,decsys3,magsat3=ntt.efoscastrodef.efoscastroloop([nameout],cat,False,40,40,100,'rxyscale',100,30,sexvec,True,10,method)
                       if rmsx3 <= 2 and rmsy3 <= 2: break
                 if rmsx3 > 2 and rmsy3 > 2:
                       for cat in ['2mass','usnoa2','usnob1']:
                             rmsx3,rmsy3,num3,fwhmgess,ellgess,ccc,rasys3,decsys3,magsat3=ntt.efoscastrodef.efoscastroloop([nameout],cat,False,20,int(20),\
                                                                                                                          int(50),'rxyscale',100,30,sexvec,True,5,method)
                             if rmsx3 <= 2 and rmsy3 <= 2: break
                       if rmsx3 > 2 and rmsy3 > 2:
                             for cat in ['2mass','usnoa2','usnob1']:
                                   rmsx3,rmsy3,num3,fwhmgess,ellgess,ccc,rasys3,decsys3,magsat3=ntt.efoscastrodef.efoscastroloop([nameout],cat,False,int(10),int(10),\
...
##########################
           ...
           try:        result=ntt.efoscastrodef.zeropoint(nameout,_system,False,False)
           except:     result=''
           if result:
                 if os.path.isfile(re.sub('.fits','.ph',nameout)): 
                       if re.sub('.fits','.ph',nameout) not in outputfile: outputfile.append(re.sub('.fits','.ph',nameout)) 
                 print '\n### zeropoint ..... done'
                 for ll in result:
           ...
           if magsat3:
                 if  readkey3(readhdr(nameout),'FLUXCAL')=='ABSOLUTE':
                       try:
                             ntt.util.updateheader(nameout,0,{'ABMAGSAT':[float(magsat3)+float(readkey3(readhdr(nameout)),'PHOTZP'),'Saturation limit for point sources (AB mags)']})
                       except:
                             ntt.util.updateheader(nameout,0,{'ABMAGSAT':[float(magsat3),'Saturation limit for point sources (AB mags)']})
                 else:                                         ntt.util.updateheader(nameout,0,{'ABMAGSAT':[float(magsat3),'Saturation limit for point sources (AB mags)']})
           else:                                         ntt.util.updateheader(nameout,0,{'ABMAGSAT':[9999.,'Saturation limit for point sources (AB mags)']})

           maglim=ntt.util.limmag(nameout)
           if maglim: ntt.util.updateheader(nameout,0,{'ABMAGLIM':[maglim,'5-sigma limiting AB magnitude for point sources']})
           else:      ntt.util.updateheader(nameout,0,{'ABMAGLIM':[9999.,'5-sigma limiting AB magnitude for point sources']})
...
################################################
                  ...
                  if int(re.sub('\.','',str(pyfits.__version__))[:2])<=30:
                        print 'here'
                        ntt.util.updateheader(img,0,{'HIERARCH ESO DET OUT1 GAIN':[1.18,'Conversion from electrons to ADU']})
                        ntt.util.updateheader(img,0,{'HIERARCH ESO DET OUT1 RON':[11.6,'Readout noise per output (e-)']})
                  else: 
                        imm=pyfits.open(img,mode='update')
                        header=imm[0].header
                        try:                    header.pop('ESO DET OUT1 GAIN')
                        except: pass
                        try:                    header.pop('ESO DET OUT1 RON')
                        except: pass
...
#################################################
                  hdr=readhdr(img)
                  if not readkey3(hdr,'pixscale'):   ntt.util.updateheader(img,0,{'pixscale':[readkey3(hdr,'binx')*.12,'pixel/scale (arcsec)']})

                  if 'FLATCOR' in hdr and os.path.isfile(hdr['FLATCOR']):
                        hdrn=readhdr(hdr['FLATCOR'])
                        if 'NCOMBINE' in hdrn:  nflat=hdrn['NCOMBINE']
                        else:   nflat=1
                  else: nflat=1
                  if 'ZEROCOR' in hdr and os.path.isfile(hdr['ZEROCOR']):
                        hdrb=readhdr(hdr['ZEROCOR'])
                        if 'NCOMBINE' in hdrb:  nbias=hdrb['NCOMBINE']
                        else:   nbias=1
                  else: nbias=1
                  ntt.util.updateheader(img,0,{'EFFRON':[readkey3(hdr,'ron') * sqrt(1. + 1./nflat + 1./nbias),'Effective readout noise per output (e-)']})
                  
                  try:
                        ntt.util.phase3header(img)  #  phase 3 definitions
                        hedvec={'quality':['Final','Final or Fast reduction'],'BUNIT':['ADU','pixel units(ADU,electrons)'],\
                                'TEXPTIME':[readkey3(hdr,'EXPTIME'),'Total integ. time of all exposure']}
                        if readkey3(hdr,'MJD-OBS'):
                              hedvec['MJD-END']=[float(readkey3(hdr,'MJD-OBS'))+(float(readkey3(hdr,'exptime'))*0.00001/(0.864)),'MJD-END = MJD-OBS + JD(exptime)']
                  ...
                  except: print 'Warning: '+img+' is not a fits file'

      f=open('logfile_phot_'+str(reduceddata)+'_'+str(datenow)+'.raw.list', 'w')
      for img in outputfile:   
            try:  f.write(readkey3(readhdr(img),'arcfile')+'\n')
            except: pass
      f.close()
...
def searchbias(img,listbias):
    ...
    if listbias:
        ...
        if len(distance)>=1:          biasfile=listbias[argmin(distance)]
        else:    biasfile=''
    else:      biasfile=''
...
def searchfringe(img,listfringe):
    ...
    if listfringe:
        ...
        if len(distance)>=1:          fringefile=listfringe[argmin(distance)]
        else:    fringefile=''
    else:      fringefile=''
...
def fringing2(img,fmask,_interactive,_verbose=False):
   ...
   if not readkey3(hdr,'FRICOR'):
         ...
         if fmask:
               ...
               delete(imgout)
               iraf.imutil.imexpr(expr='a - ('+str(scalevalue)+'* (b - '+str(median(matfrin))+') )',a=img, b=maskname, output=imgout, verbose='no')
               print '\n### fringing correction  ..... done'
               iraf.hedit(images=imgout,fields='OBJMASK',value='',delete='yes',update='yes',verify='no')
               ntt.util.updateheader(imgout,0,{'FRICOR':[str(scalevalue)+' * ('+str(maskname)+' - '+str(median(matfrin))+')','']})
