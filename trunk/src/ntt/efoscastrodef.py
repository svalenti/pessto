...
#        CDELT2=2.
        ...
        if not CRPIX1:
            ...
            if _CRPIX1:    CRPIX1=float(_CRPIX1)
            else:     CRPIX1= 512.
        else:  CRPIX1= float(CRPIX1)+512

        if not CRPIX2:
            _CRPIX2=readkey3(hdr,'NTCRPIX2')#+10
            if _CRPIX2:    CRPIX2=float(_CRPIX2)
            else:          CRPIX2= 512.
        else:  CRPIX2= float(CRPIX2)+521
#        CDELT1=1.
#        CDELT2=1.
        CD1_1=CDELT0*cos(theta)
    ...
    ntt.util.updateheader(img,0,{'CTYPE1': [CTYPE1,'pixel coordinate system'],       'CTYPE2': [CTYPE2,'pixel coordinate system'],\
                                 'CRVAL1': [CRVAL1,'RA at ref pixel'],               'CRVAL2': [CRVAL2,'DEC at ref pixel'],\
                                 'CRPIX1': [CRPIX1,'Ref pixel in X'],                'CRPIX2': [CRPIX2,'Ref pixel in Y'],\
                                 'CD1_1' : [CD1_1,'Transformation matrix element'],  'CD2_2' : [CD2_2,'Transformation matrix element'],\
                                 'CD1_2' : [CD1_2,'Transformation matrix element'],  'CD2_1' : [CD2_1,'Transformation matrix element'],\
                                 'WCSDIM': [WCSDIM,'']})


def efoscastroloop(imglist,catalogue,_interactive,number1,number2,number3,_fitgeo,_tollerance1,_tollerance2,sexvec='',_guess=False,_numin=4,method='iraf',_CRPIX1='',_CRPIX2=''):
    import ntt
...
###################
        ...
        rmsx1,rmsy1,num1,fwhm1,ell1,ccc,bkg1,rasys1,decsys1=efoscastrometry2([img],catalogue,_interactive,number1,sexvec,catvec,guess=False,fitgeo=_fitgeo,\
                                                                                 tollerance1=_tollerance1, tollerance2=_tollerance2,_update='yes',imex=_imex,nummin=_numin)
        if rmsx1>1 or rmsy1>1:
            catvec=ntt.efoscastrodef.querycatalogue(catalogue,img,method)
            rmsx2,rmsy2,num2,fwhm2,ell2,ccc,bkg2,rasys2,decsys2=efoscastrometry2([img],catalogue,_interactive,number2,sexvec,catvec,guess=False,fitgeo=_fitgeo,\
                                                                                     tollerance1=_tollerance1, tollerance2=_tollerance2,_update='yes',imex=_imex,nummin=_numin)
            if rmsx2>1 or rmsy2>1:
                catvec=ntt.efoscastrodef.querycatalogue(catalogue,img,method)
                rmsx3,rmsy3,num3,fwhm3,ell3,ccc,bkg3,rasys3,decsys3=efoscastrometry2([img],catalogue,_interactive,number3,sexvec,catvec,guess=False,fitgeo=_fitgeo,\
                                                                                         tollerance1=_tollerance1, tollerance2=_tollerance2,_update='yes',imex=_imex,nummin=_numin)
...
######################################## 
        if rmsx3 < 10 and rmsy3 < 10: 
            if _instrume=='efosc':
                ...
                if _imex:  fwhmgessime = median(array(ccc))*readkey3(hdr,'binx')*.12
                else:     fwhmgessime = 9999
            elif _instrume=='sofi':
                fwhmgess3=median(array(fwhm3))*.68*2.35*0.288
                if _imex:  fwhmgessime = median(array(ccc))*0.288
                else:     fwhmgessime = 9999
        ...
        if fwhmgess3:
            ...
            if _instrume=='efosc': 
                ...
                magsat=-2.5*math.log10(  (math.pi/(4*math.log(2.)))*(32000.-float(mbkg3))*((float(fwhmgess3)/0.288)**2)  )
        else:        magsat=9999
...
def zeropoint(img,_field,verbose=False,_interactive=False):
    ...
    kk={'U':0.46,'u':0.46,'B':0.27,'g':0.20,'V':0.12,'r':0.09,'R':0.09,'i':0.02,'I':0.02,'z':0.03, 'J':0.0,'H':0.0, 'K':0.0}
    zz={'U640':23.69,'B639':25.83,'V641':25.88,'R642':25.98,'r784':25.27,'i705':25.13,'g782':25.86,'z623':24.41,'J':25,'H':25,'Ks':25}
    ...
    if _filter in ['J', 'Js', 'H', 'Ks','K']:                     
        ...
        for ww in colasci:
                for jj in range(0,len(stdcoo[ww])):
                    try: stdcoo[ww][jj]=float(re.sub('L','',stdcoo[ww][jj]))
                    except: stdcoo[ww][jj]=999
        ...
        delete('tmp.stdL.pix')
        iraf.wcsctran(ntt.__path__[0]+'/standard/cat/landolt.cat','tmp.stdL.pix',img,inwcs='world',units='degrees degrees',outwcs='logical',\
...
#standardpix={'ra':[9999],'dec':[9999],'id':[1]}
    ...
    if len(xstd0)>1:  ########   go only if standard stars are in the field  ##########
        ...
        distvec,pos0,pos1=ntt.efoscastrodef.crossmatch(array(rastd0),array(decstd0),array(rasex),array(decsex),10)
...
#                mag=mag0-kk[filters[_filter]]*float(_airmass)
        ...
        for col in colorvec:
            ...
            if len(colore)==0:
                ...
                if not _interactive:
                    ...
                    ntt.util.updateheader(img,0,{'FLUXCAL':['ABSOLUTE','Certifies the validity of PHOTZP']})
                    if _field=='landolt':      a, b, RR=ntt.efoscastrodef.linreg(colore, zero)
                    elif _field=='2mass':      b,a,RR=mean(zero),0,0 
                    elif _field=='sloan':      b,a,RR=mean(zero),0,0
    ...
    if _field=='filter':     
        if _filter in ['J', 'Js', 'H', 'Ks','K']:  _field='2mass'
        elif _filter in ['U640', 'B639', 'V641', 'R642','i705']:  _field='landolt'
        elif _filter in ['g782','r784','z623']:  _field='sloan'
    if _field=='landolt':     ntt.util.updateheader(img,0,{'PHOTSYS':['VEGA','Photometric system VEGA or AB']})
    elif _field=='2mass':     ntt.util.updateheader(img,0,{'PHOTSYS':['VEGA','Photometric system VEGA or AB']})
    elif _field=='sloan':     ntt.util.updateheader(img,0,{'PHOTSYS':['AB','Photometric system VEGA or AB']})
    else:                     ntt.util.updateheader(img,0,{'PHOTSYS':['NULL','Photometric system VEGA or AB']})
...
def crossmatch(_ra0,_dec0,_ra1,_dec1,tollerance):  #   degree,degree,degree,degree,arcsec
    ...
    for jj in range(0,len(_ra0)):
        distance=arccos(sin(array(_dec1)*scal)*sin(_dec0[jj]*scal)+cos(array(_dec1)*scal)*cos(_dec0[jj]*scal)*cos((array(_ra1)-_ra0[jj])*scal))
...
#        pol[1]=std(array(_dmag)-array(_col)*float(_fixcol))
    ...
    try:      sigmae = np.sqrt(pol[1]/(len(idd)  -2))
    except:   sigmae=0
    sigmaa = sigmae*np.sqrt(1./len(idd)+(np.mean(_col[idd])**2)/np.sum((_col[idd]-\
                                    np.mean(_col[idd]))**2))
...
def querysloan(ra1,dec1,radius,mr1,mr2):
    ...
    righe = sqlcl.query('select P.objID, P.ra, P.dec , P.u, P.g, P.r, P.i, P.z, P.type '+'from PhotoPrimary as P , dbo.fGetNearbyObjEq('+\
...
def sloan2file(ra1, dec1, radius, magsel0, magsel1,_output):
    ...
    _ids,_ras,_decs,_us,_gs,_rs,_is,_zs,_type=ntt.efoscastrodef.querysloan(ra1,dec1,float(radius),magsel0,magsel1)
    ...
    if _ids:
         ...
         for i in range(0,len(_idss)):
             ff.write('%12.12s\t%12.12s\t%s\t%8.8s\t%8.8s\t%8.8s\t%8.8s\t%8.8s\n' %\
                          (str(_rass[i]),str(_decss[i]),str(_idss[i][-6:]),str(_uss[i]),str(_gss[i]),str(_rss[i]),str(_iss[i]),str(_zss[i])))
         ff.close()
    else: _idss=''
    return _idss
#######################################################################
...
############################################################################################################
def efoscastrometry2(lista,catalogue,_interactive,number,sexvec,catvec,guess=False,fitgeo='xyscale', tollerance1=100, tollerance2=30, _update='yes',imex=False,nummin=4):
...
#    if guess:     ntt.efoscastrodef.wcsstart(img)#,500,500)
    ...
    if verbose:
            iraf.tvmark(1,'STDIN',Stdin=list(apix1),mark="circle",number='yes',label='no',radii=20,nxoffse=5,nyoffse=5,color=205,txsize=4)
...
##############    sextractor   ##################
    ...
    if verbose:
            ...
            iraf.tvmark(1,'STDIN',Stdin=list(sexpix),mark="circle",number='yes',label='no',radii=8,nxoffse=5,nyoffse=5,color=206,txsize=2)
...
#####################################################################
    ...
    if len(xref)>=nummin:
            _ccmap1=iraf.ccmap('STDIN','STDOUT',images=img,Stdin=vettoretran,fitgeome=fitgeo,xcolum=3, xxorder=2, yyorder=2, ycolum=4,lngcolum=1,latcolumn=2,lngunit='degrees',update='No',interact='No',maxiter=3,Stdout=1)
            if 'rms' in _ccmap1[_ccmap1.index('Wcs mapping status')+1]:
                try:    
                    rmsx,rmsy=array(string.split(string.split(_ccmap1[_ccmap1.index('Wcs mapping status')+1],':')[-1])[0:2],float)
                    if float(rmsx)<2. and float(rmsy)<2.:
                        _ccmap1=iraf.ccmap('STDIN','STDOUT',images=img,Stdin=vettoretran,fitgeome=fitgeo,xcolum=3, xxorder=2, yyorder=2, ycolum=4,lngcolum=1,latcolumn=2,lngunit='degrees',update='Yes',interact='No',maxiter=3,Stdout=1)
