def makeflat(lista):
    ...
    toforget = ['imgeom.blkavg', 'imutil.imarith','immatch.imcombine','noao.imred']
    ...
    output=name_duplicate(lista[3],'flat_'+str(_date)+'_'+str(_filter)+'_'+str(MJDtoday),'')
    ...
    if answ in ['yes','y','YES','Y','Yes']:
        ...
        iraf.image.immatch.imcombine(lista[0]+','+lista[7],output="temp_off.fits")
        iraf.image.immatch.imcombine(lista[1]+','+lista[6],output="temp_off_mask.fits")
        iraf.image.immatch.imcombine(lista[2]+','+lista[5],output="temp_on_mask.fits")
        iraf.image.immatch.imcombine(lista[3]+','+lista[4],output="temp_on.fits")
        ...
        iraf.imgeom.blkavg(input="temp_on.fits[500:600,*]",output="temp_onA.fits",option="average",b1=101,b2=1)
        iraf.imgeom.blkavg(input="temp_on_mask.fits[500:600,*]",output="temp_onC.fits",option="average",b1=101,b2=1)
        iraf.imgeom.blkavg(input="temp_on_mask.fits[50:150,*]",output="temp_onB.fits",option="average",b1=101,b2=1)
        iraf.imutil.imarith("temp_onA.fits","-","temp_onC.fits","temp_onAC.fits")
        iraf.imutil.imarith("temp_onAC.fits","+","temp_onB.fits","temp_onACB.fits")
        iraf.imgeom.blkrep(input="temp_onACB.fits",output="temp_onACB_2D.fits",b1=1024,b2=1)
        iraf.imutil.imarith("temp_on.fits","-","temp_onACB_2D.fits","temp_on_bias.fits")
    #   same as above for the flat-off
        ...
        iraf.imgeom.blkavg(input="temp_off.fits[500:600,*]",output="temp_offA.fits",option="average",b1=101,b2=1)
        iraf.imgeom.blkavg(input="temp_off_mask.fits[500:600,*]",output="temp_offC.fits",option="average",b1=101,b2=1)
        iraf.imgeom.blkavg(input="temp_off_mask.fits[50:150,*]",output="temp_offB.fits",option="average",b1=101,b2=1)
        iraf.imutil.imarith("temp_offA.fits","-","temp_offC.fits","temp_offAC.fits")
        iraf.imutil.imarith("temp_offAC.fits","+","temp_offB.fits","temp_offACB.fits")
        iraf.imgeom.blkrep(input="temp_offACB.fits",output="temp_offACB_2D.fits",b1=1024,b2=1)
        iraf.imutil.imarith("temp_off.fits","-","temp_offACB_2D.fits","temp_off_bias.fits")
    ...
    #    print lista[0],'flat_'+str(_date)+'_'+str(_filter)+'_'+str(MJDtoday)
        delete(output)
        iraf.imutil.imarith("temp_on_bias.fits","-","temp_off_bias.fits",output)
    ...
    else: print 'skip redoing the flat'
    return output
...
def makeillumination(lista,flatfield):#,outputfile,illum_frame):
    ...
    _date=readkey3(readhdr(lista[0]),'date-night')
    _filter=readkey3(readhdr(lista[0]),'filter')
    illum_frame=name_duplicate(lista[0],'illum_'+_date+'_'+_filter+'_'+str(MJDtoday),'')
    from pyraf import iraf
    iraf.images(_doprint=0)
    iraf.imutil(_doprint=0)
    ...
    iraf.generic(_doprint=0)
    toforget = ['digiphot.daophot', 'imutil.imarith','image','utilities.surfit']
    for t in toforget: iraf.unlearn(t)
    n=len(lista)
#   start loop to read image names from the input file
    ...
    lista2,skyfile=ntt.sofiphotredudef.skysub(lista1,readkey3(readhdr(lista1[0]),'ron'),readkey3(readhdr(lista1[0]),'gain'),True)
    lista2=ntt.sofiphotredudef.sortbyJD(lista2)
    print '\n### use x on the star and q  to continue....'
    display_image(lista2[0],2,'','',False)
    delete('tmpone.coo')
    iraf.image.tv.imexamine(lista2[0],2,logfile='tmpone.coo',keeplog='yes',xformat='', yformat='' , wcs= 'logical')
    iraf.tvmark(2,'tmpone.coo',mark="circle",number='yes',label='no',radii=8,nxoffse=5,nyoffse=5,color=204,txsize=2)
    ...
    for i in range(0,len(lista2)):
        ...
        iraf.tvmark(1,'tmpone.coo',mark="circle",number='yes',label='no',radii=8,nxoffse=5,nyoffse=5,color=204,txsize=2)
        answ='n'
        while answ!='y':
            ...
            if answ in ['y','YES','yes','Y']:
                ...
                gggg=iraf.digiphot.daophot.phot(lista2[i],"tmpone.coo",output="pippo."+str(j)+".mag",verify='no',interac='no',Stdout=1)
                try:  
                ...
                print '\n### select the std star'
                display_image(lista2[i],1,'','',False)
                iraf.image.tv.imexamine(lista2[i],1,logfile='tmpone.coo',keeplog='yes',xformat='', yformat='' , wcs= 'logical')
                xycoo = iraf.proto.fields('tmpone.coo','1,2',Stdout=1)
    ...
    tmptbl0=iraf.txdump(textfile="@tempmag.lst",fields="XCENTER,YCENTER,FLUX",expr='yes',Stdout=1)
...
#   delete the temporary images and files
    ...
    aaa=iraf.utilities.surfit('magnitudini',image=illum_frame,function="polynomial",xorder=2,yorder=2,xterms="full",ncols=1024,nlines=1024,Stdout=1)
    ...
    ntt.util.updateheader(illum_frame,0,{'MKILLUM':[flatfield0,'flat field']})
...
def doflatsofi(flats,_doflat,illum,_output):
    ...
    masklimit={'J':{'ON':1000,'OFF':30},'H':{'ON':1000,'OFF':40},'Ks':{'ON':1000,'OFF':1000}}
        ...
        for _filter in flats:
            for ID in flats[_filter]:
                ...
                if len(images)==8:
                    ...
                    tipo=['OFF','OFFMASK','ONMASK','ON','ON','ONMASK','OFFMASK','OFF']
                    listtmp=[]
                    ii=0
                    nn=0
                    for img in images:
                        onoffvalue=float(string.split(iraf.imstat(img+'[500:600,900:1000]',Stdout=1)[1])[2])
                        maskvalue=float(string.split(iraf.imstat(img+'[100:200,900:1000]',Stdout=1)[1])[2])
                        if onoffvalue>=onofflimit[_filter]: onoff='ON'
                        else: onoff='OFF'
                        if maskvalue>= masklimit[_filter][onoff]:  mask='none'
                        else: mask='MASK'
    ...
    if illum:
        for _filter in illum:
            for ID in illum[_filter]:
                ...
                if flatfield:
                    illum_frame=ntt.soficalibdef.makeillumination(images,flatfield)
                    listaillum.append(illum_frame)
                else:        print 'flat field not found'

        for img in listaflat:
            try:
                ntt.util.phase3header(img)  #  phase 3 definitions
                ntt.util.updateheader(img,0,{'BUNIT':['ADU','pixel units(ADU,electrons)']})
                ntt.util.updateheader(img,0,{'FILETYPE':[31202,'flat field']})
        ...
        for img in listaillum:
            try:
                ntt.util.phase3header(img)  #  phase 3 definitions
                ntt.util.updateheader(img,0,{'BUNIT':['ADU','pixel units(ADU,electrons)']})
                ntt.util.updateheader(img,0,{'FILETYPE':[31213,'illum corr  frames']})
            except: print '\n### problems with phase 3 definitions'
