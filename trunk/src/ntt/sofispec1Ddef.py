...
def findaperture(img, _interactive=False):
    ...
    try:
        for line in open('database/ap' + re.sub('.fits', '', img)):
            if "center" in line:   center = float(string.split(line)[1])
    except:
        center = 9999
    return center
...
def calibrationsofi(imgex, stdex, stdref, outputfile, _interactive):
    ...
    dataref, hdrref = pyfits.getdata(stdref, 0, header=True)  # read refrence standard
    ...
    datastd, hdrstd = pyfits.getdata(stdex, 0, header=True)  # read tellurich standard
    ...
    if 'sun' in stdref:
        yystd_sc = (datastd[0][0] / _exptimestd) / ( (10 ** ((-27.47) / 2.5)) / (10 ** ((_magstd) / 2.5)))
    elif 'vega' in stdref:
        yystd_sc = (datastd[0][0] / _exptimestd) / ( (10 ** ((0.0023) / 2.5)) / (10 ** ((_magstd) / 2.5)))
    else:
    ...
    for imcmb in matching:
        aaa = iraf.hedit(senstelluric, imcmb, delete='yes', update='yes', verify='no', Stdout=1)
    ntt.util.updateheader(senstelluric, 0, {'TRACE1': [stdex, 'Originating file']})
...
def sofispec1Dredu(files, _interactive, _ext_trace, _dispersionline, _automaticex, _verbose=False):
    ...
    std_sun, rastd_sun, decstd_sun, magstd_sun = ntt.util.readstandard('standard_sofi_sun.txt')
    std_vega, rastd_vega, decstd_vega, magstd_vega = ntt.util.readstandard('standard_sofi_vega.txt')
    std_phot, rastd_phot, decstd_phot, magstd_phot = ntt.util.readstandard('standard_sofi_phot.txt')
    outputfile = []
    objectlist, RA, DEC = {}, {}, {}
    for img in files:
        img = re.sub('\n', '', img)
    ...
    for _type in objectlist:
        for setup in objectlist[_type]:
            for _ID in objectlist[_type][setup]:
                ...
                _object = ntt.util.readkey3(ntt.util.readhdr(listmerge[0]), 'object')
                if string.count(_object, '/') or string.count(_object, '.') or string.count(_object, ' '):
                    ...
                    nameobj = _object
                _date = ntt.util.readkey3(ntt.util.readhdr(listmerge[0]), 'date-night')
                outputimage = nameobj + '_' + _date + '_' + setup + '_merge_' + str(MJDtoday)
                outputimage = ntt.util.name_duplicate(listmerge[0], outputimage, '')
                ...
                if answ in ['Yes', 'y', 'Y', 'yes']:
                    ...
                    if automaticmerge == 'yes':
                        offset = 0
                        offsetvec = []
                        _center0 = ntt.sofispec1Ddef.findaperture(listmerge[0], False)
                        _offset0 = ntt.util.readkey3(ntt.util.readhdr(listmerge[0]), 'xcum')
                        print '\n### Try to merge spectra considering their offset along x axes .......'
                        f = open('_offset', 'w')
                        for img in listmerge:
                            _center = ntt.sofispec1Ddef.findaperture(img, False)
                            _center2 = (float(_center) + (float(_offset0) - float(_center0))) * (-1)
                            _offset = (-1) * ntt.util.readkey3(ntt.util.readhdr(img), 'xcum')
                    ...
                    if automaticmerge == 'yes':
                        ...
                        offset = 0
                        offsetvec = []
                        _z1, _z2, goon = ntt.util.display_image(listmerge[0], 1, '', '', False)
                        print '\n### find aperture on first frame and use it as reference position of ' \
                              'the spectra (mark with ' + '"' + 'm' + '"' + ')'
                        _center0 = ntt.sofispec1Ddef.findaperture(listmerge[0], True)
                        _offset0 = ntt.util.readkey3(ntt.util.readhdr(listmerge[0]), 'xcum')
                        ...
                        for img in listmerge:
                            ...
                            if not offset3: offset3 = _center2
                            offsetvec.append(offset3)
                            line = str(offset3) + '   0\n'
                            f.write(line)
                        f.close()
                    ...
                    iraf.imutil.imcopy('_output[' + str(start) + ':1024,*]', output=outputimage, verbose='no')
                    print outputimage
                    print len(listmerge)
                    ...
                    for imcmb in matching:   aaa = iraf.hedit(outputimage, imcmb, delete='yes', update='yes',
                                                              verify='no', Stdout=1)
                    if 'SKYSUB' in hdr.keys(): aaa = iraf.hedit(outputimage, 'SKYSUB', delete='yes', update='yes',
                                                                verify='no', Stdout=1)
                    ...
                    for img in listmerge:
                        ...
                        ntt.util.updateheader(outputimage, 0,
                                              {'PROV' + str(num):
                                                   [ntt.util.readkey3(hdrm, 'ARCFILE'), 'Originating file'],
                ...
                if outputimage not in outputfile: outputfile.append(outputimage)
                ntt.util.updateheader(outputimage, 0, {'FILETYPE': [42116, 'combine 2D spectra frame']})

    if _verbose:
        if 'obj' in objectlist: print objectlist['obj']
        if 'stdp' in objectlist: print objectlist['stdp']
        if 'sun' in objectlist: print objectlist['sun']
        if 'vega' in objectlist: print objectlist['vega']

    if 'obj' not in objectlist.keys(): sys.exit('\n### error: no objects in the list')
    ...
    for setup in objectlist['obj']:
        ...
        for _ID in objectlist['obj'][setup]:
            for img in objectlist['obj'][setup][_ID]:
                ...
                for imcmb in matching:
                    aaa = iraf.hedit(imgex, imcmb, delete='yes', update='yes', verify='no', Stdout=1)
                ...
                if _type in ['sun', 'vega']:
                    stdref = ntt.__path__[0] + '/standard/fits/' + str(_type) + '.fits'
                    stdvec, airmassvec, JDvec = [], [], []
                    for _ID in objectlist[_type][setup]:
                        for std in objectlist[_type][setup][_ID]:
                            _airmassstd = ntt.util.readkey3(ntt.util.readhdr(std), 'airmass')
                            _JDstd = ntt.util.readkey3(ntt.util.readhdr(std), 'JD')
                            JDvec.append(abs(_JDstd - _JDimg))
                            stdvec.append(std)
                            airmassvec.append(_airmassstd)
                    stdtelluric = stdvec[np.argmin(JDvec)]
                    _exptimestd = ntt.util.readkey3(ntt.util.readhdr(stdtelluric), 'exptime')
                    _magstd = ntt.util.readkey3(ntt.util.readhdr(stdtelluric), 'magstd')
                    ...
                    stdtelluric_ex = ntt.util.extractspectrum(stdtelluric, dv, False, False, _interactive, 'std',
                                                              automaticex=_automaticex)
                    if stdtelluric_ex not in outputfile: outputfile.append(stdtelluric_ex)
                    ntt.util.updateheader(stdtelluric_ex, 0, {'FILETYPE': [42107, 'extracted 1D wave calib ']})
                    ...
                    matching = [s for s in hdr.keys() if "TRACE" in s]
                    for imcmb in matching:
                        aaa = iraf.hedit(stdtelluric_ex, imcmb, delete='yes', update='yes', verify='no', Stdout=1)
                    ntt.util.updateheader(stdtelluric_ex, 0, {'TRACE1': [stdtelluric, 'Originating file']})
                    ##############################################################################################
                    ...
                    if senstelluric not in outputfile: outputfile.append(senstelluric)
...
#                                                    'SNR': [ntt.util.StoN(imgf, 50),
                    ...
                    #############################################################################################
                    imgd = ntt.efoscspec1Ddef.fluxcalib2d(img, senstelluric)  # flux calibration 2d images
                    ntt.util.updateheader(imgd, 0, {'FILETYPE': [42209, '2D wavelength and flux calibrated spectrum']})
                    ...
                    for imcmb in matching:
                        aaa = iraf.hedit(imgd, imcmb, delete='yes', update='yes', verify='no', Stdout=1)
                    ntt.util.updateheader(imgd, 0, {'TRACE1': [img, 'Originating file']})
                ...
                if 'stdp' in objectlist and setup in objectlist['stdp']:
                    ...
                    for _ID in objectlist['stdp'][setup]:
                        for stdp in objectlist['stdp'][setup][_ID]:
                            ...
                            if stdp_ex not in outputfile: outputfile.append(stdp_ex)
                    ...
                    if len(standardfile) >= 2:
                        standardfile0 = raw_input('which one do you want to use [' + str(standardfile[0]) + '] ? ')
                        if not standardfile0: standardfile0 = standardfile[0]
                    else:
                    ...
                    stdname = ntt.util.readkey3(ntt.util.readhdr(standardfile0), 'stdname')
                    ...
                    pl.clf()
                    pl.ion()
                    pl.plot(aa1, datastdpf[0][0] / yystd, color='red', label='std')
                    ...
                    if imgsc not in outputfile: outputfile.append(imgsc)
                else:
